...
// Note on the previous `LOG_SKIP_GET_ROUTES`/`LOG_SKIP_4XX_ROUTES`/
// `shouldSkipLogging`: those were tied to the old "log every request"
// model. After the event-log refactor, the interceptor only emits a
// `perf.slow_request` event (≥ 2s) and the filter only emits selected
// security/error events (5xx, 403, 429) — both are inherently low-volume
// and don't need URL-based denylists. The functions stayed exported in
// case future code wants per-route opt-outs; they aren't called from
// the request-logging hot path anymore.
